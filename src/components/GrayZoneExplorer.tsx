...
const TABS: TabDef[] = [
  {
    ...
    tipColor: "border-orange-500/20 bg-orange-950/20 text-orange-300/80",
  },
  {
    key: "private",
    ...
    tipColor: "border-red-500/20 bg-red-950/20 text-red-300/80",
  },
  {
    key: "streaming",
    label: "Streaming",
    ...
    tipColor: "border-purple-500/20 bg-purple-950/20 text-purple-300/80",
  },
  {
    key: "ebooks",
    label: "Books & Papers",
    ...
    tipColor: "border-blue-500/20 bg-blue-950/20 text-blue-300/80",
  },
  {
    key: "music",
    label: "Music",
    ...
    tipColor: "border-pink-500/20 bg-pink-950/20 text-pink-300/80",
  },
  {
    key: "software",
    ...
    tipColor: "border-cyan-500/20 bg-cyan-950/20 text-cyan-300/80",
  },
];
...
function GrayExplorerCard({ item }: { item: GrayAlternative }) {
  ...
  return (
    <a
      ...
      className="group flex flex-col gap-2.5 rounded-xl border border-white/8 bg-white/5 p-4 hover:bg-white/10 hover:border-white/20 transition-all duration-200"
    >
      ...
      <div className="flex flex-wrap gap-1">
        {item.tags.map((tag) => (
          <span key={tag} className="rounded-full bg-white/8 px-2 py-0.5 text-xs text-gray-500 font-medium">
            {tag}
          </span>
        ))}
      </div>
      ...
      <div className="flex items-center gap-1 text-xs text-orange-400/60 group-hover:text-orange-400 transition-colors mt-auto pt-1 font-semibold">
        Visit site
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-orange-400/70">{totalItems} resources · For educational purposes only</p>
                </div>
              </div>

              <div className="flex items-center gap-3">
                <div className="hidden sm:flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-950/30 px-3 py-1.5">
                  <span className="text-xs">🛡️</span>
                  <span className="text-xs text-amber-300/80 font-medium">Use a VPN</span>
                </div>
                <button
                  onClick={onClose}
            ...
            {isReady ? (
              <div className="flex flex-1 overflow-hidden">
                {/* Sidebar tabs */}
                <div className="hidden sm:flex flex-col w-52 shrink-0 border-r border-white/8 py-4 px-3 gap-1 overflow-y-auto">
                  {TABS.map((t, idx) => (
                    <button
                      key={t.key}
                    ...
                    >
                      ...
                      <span className={`rounded-full px-1.5 py-0.5 text-xs font-bold ${
                        activeTab === idx ? "bg-white/20 text-white" : "bg-white/8 text-gray-500"
                      }`}>
                        {t.items.length}
                      </span>
                    </button>
                  ...
                  <div className="mt-4 rounded-xl border border-amber-500/20 bg-amber-950/20 p-3">
                    ...
                    <ul className="space-y-1.5 text-xs text-amber-300/60">
                      <li>• VPN always on</li>
                ...
                {/* Mobile tab bar */}
                <div className="sm:hidden fixed bottom-0 left-0 right-0 flex gap-1 px-2 py-2 bg-gray-950 border-t border-white/10 z-10 overflow-x-auto">
                  {TABS.map((t, idx) => (
                    <button
                      key={t.key}
                      onClick={() => setActiveTab(idx)}
                ...
                {/* Content area */}
                <div className="flex-1 overflow-y-auto px-5 py-6 pb-24 sm:pb-6">
